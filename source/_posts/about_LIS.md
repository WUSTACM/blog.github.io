...
对于求一个数列的最长递增子序列，<!-- more -->我们可以在外维护一个数组$a$，用来存放递增序列。从前到后遍历每一个$v[i]$，对于每一个$v[i]$，使用二分查找寻找其能在数组$a$中所能替换的位置，即第一个大于等于$v[i]$的位置，将其替换为$v[i]$。若$v[i]$大于数组中的所有数字，即可将其放到最后，此时数组长度会加一。答案即是数组$a$中有效的长度。
